...
function enemy:tap( event)
	transition.to(pers,{time=850, x = enemy.x -40, y = enemy.y, onComplete = audio.play(soco_mp3)})
	enemy.hp = enemy.hp - 1
	if( enemy.hp <=0) then
...
function enemy2:tap( event)
	transition.to(pers,{time=850, x = enemy2.x -40, y = enemy2.y, onComplete = audio.play(soco_mp3)}) 
	audio.play(soco_mp3)
